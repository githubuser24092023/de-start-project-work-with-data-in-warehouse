...
select 
        ...
        t2.product_type,
        t2.product_price
  from external_source.customers t1
    join external_source.craft_products_orders t2
...
WHEN MATCHED THEN
  UPDATE SET 
            ...
            /* craftsman_birthday = t.craftsman_birthday, */ -- CANNOT be changed (?) 
...
WHEN MATCHED THEN
  UPDATE SET 
            ...
            /* customer_birthday = t.customer_birthday, */ -- CANNOT be changed (?) 
            load_dttm = current_timestamp
WHEN NOT MATCHED THEN
...
SELECT  
  ...
  FROM tmp_sources src
    JOIN dwh.d_craftsman dc 
      ON dc.craftsman_name = src.craftsman_name and dc.craftsman_email = src.craftsman_email 
	JOIN dwh.d_customer dcust 
	  ON dcust.customer_name = src.customer_name and dcust.customer_email = src.customer_email 
	JOIN dwh.d_product dp 
      ON dp.product_name = src.product_name and dp.product_description = src.product_description /*and dp.product_price = src.product_price*/;
